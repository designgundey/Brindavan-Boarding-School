import { Renderer, Program, Mesh, Triangle, Texture } from 'https://cdn.jsdelivr.net/npm/ogl@1.0.6/src/index.js';
...
const drawLine = (ctx, line, x, y, letterSpacing, highlight) => {
  ...
  if (highlight) {
    ...
    const beforeWidth = before ? measureLine(ctx, before, letterSpacing) + letterSpacing : 0;
    const highlightWidth = measureLine(ctx, highlight, letterSpacing);
    const metrics = ctx.measureText('M');
    const padY = (metrics.actualBoundingBoxAscent || 0) * 0.28;
    const padX = padY * 1.1;
...
export function initWarpText(container, userProps = {}) {
...
document.addEventListener('DOMContentLoaded', () => {
  const el = document.querySelector('.lead-warp');
  if (!el) return;
  initWarpText(el, {
    text: 'Education is about who\na child becomes, as much\nas what they learn.',
    color: '#193de8',
